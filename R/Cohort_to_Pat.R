Cohort.to.Pat <- function(cumu.data, ref.dose, variable.names)
{
  ...
  cohort.DLT <- data.table(cumu.data)[, .(DLT = c(rep(1, .SD[[DLT.name]]), rep(0, .SD[[npat.name]]-.SD[[DLT.name]]))), by = seq_len(nrow(cumu.data)), .SDcols = c(DLT.name, npat.name)][, "DLT"] %>% setnames("DLT", DLT.name)
